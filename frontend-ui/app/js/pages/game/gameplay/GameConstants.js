// const infoCtnHeight = document.querySelector('.info-ctn').offsetHeight;

// console.log("window.innerHeight: ", window.innerHeight);
// console.log("window.innerWidth: ", window.innerWidth);

export const canvasHeight = 550;
export const canvasWidth = 1100;
// console.log("canvasHeight, canvasWidth: ", canvasHeight, canvasWidth);
export const ballRadius = 10;
export const paddleWidth = 15;
export const paddleHeight = 100;

export const controllerRightUp = "j";
export const controllerRightDown = "n";
export const controllerLeftUp = "f";
export const controllerLeftDown = "v";

//need to fetch the backend values through api