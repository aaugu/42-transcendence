...
export async function blockUser(target_id) {
	...
	const response = await fetch('https://localhost:10443/livechat/' + userID + '/blacklist/', {
		method: 'POST',
		headers: {
...
export async function unblockUser(target_id) {
	...
	const response = await fetch('https://localhost:10443/livechat/' + userID + '/blacklist/' + target_id, {
		method: 'DELETE',
		headers: {
			'Accept': 'application/json',
			'Content-Type': 'application/json',
