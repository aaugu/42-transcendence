...
class ApiChatConsumer(AsyncWebsocketConsumer):
	async def connect(self):
		...
		try:
			...
			await self.close(4004, "Service unavailable or conversation does not exists")
	...
	async def receive(self, text_data):
		try:
			...
			await self.close(4000, "Service unavailable or you are not authorized to proceed")

		try:	
			print(response_text_data)
			text_data_json = json.loads(response_text_data)

			# if (text_data_json['error']):
			# 	self.close(1000, text_data_json['error'])
